    ...
    <form action="{{ route('admin.role.store') }}" method="POST">
        ...
        <div class="flex flex-col items-start justify-center gap-5 w-full">
            <input type="text" name="title"
                   ...
                   placeholder="Название" spellcheck="true" autofocus>
            @error('title')
            ...
            <div class="flex items-center justify-start gap-3">
                <button type="submit" class="p-2 bg-[#6366f1] text-white rounded-md">Создать</button>
                <a href="{{ route('admin.index') }}" class="p-2 bg-[#efecff] text-black rounded-md">Назад</a>
            </div>
        </div>
    </form>
@endsection
